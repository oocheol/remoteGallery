...
export async function GET(_request: NextRequest, context: Context) { try { const saved = await snapshot((await context.params).token); saved.artworks = saved.artworks.map(art => ({ ...art, matSizing: art.matSizing ?? "outset", frameSizing: art.frameSizing ?? "outset" })); return NextResponse.json(saved, { headers: { 'Cache-Control': 'no-store' } }); } catch (error) { return apiError(error); } }
